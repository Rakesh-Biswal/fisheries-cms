// components/Hrcomponent/holiday-manager.js
"use client"

import React, { useState, useEffect, useRef } from 'react';
import dynamic from 'next/dynamic';

// Dynamically import FullCalendar to avoid SSR issues with proper error handling
const FullCalendar = dynamic(
    () => {
        if (typeof window !== 'undefined') {
            return import('@fullcalendar/react').then(mod => {
                // Wrap the component to handle cleanup properly
                const WrappedCalendar = React.forwardRef((props, ref) => {
                ...
                WrappedCalendar.displayName = 'WrappedFullCalendar';
                return WrappedCalendar;
            });
        }
        return Promise.resolve(() => <div>Loading calendar...</div>);
    },
    {
        ssr: false,
        loading: () => (
...
// Dynamically import plugins with error handling
const loadPlugins = async () => {
    if (typeof window === 'undefined') return { dayGridPlugin: null, interactionPlugin: null };
    ...
    try {
        ...
        console.error('Error loading calendar plugins:', error);
        return { dayGridPlugin: null, interactionPlugin: null };
    }
};
...
const HolidayManager = () => {
    ...
    const [plugins, setPlugins] = useState({ dayGridPlugin: null, interactionPlugin: null })
    ...
    useEffect(() => {
        const initializeCalendar = async () => {
            try {
                ...
                setCalendarReady(true); // Still set ready to show error state
            }
        };
    ...
    const filteredEvents = events.filter(event => {
        ...
        if (searchTerm) {
            ...
            const matchesSearch =
                ...
                event.extendedProps.description?.toLowerCase().includes(searchLower) ||
                event.extendedProps.departments?.some(dept =>
                    getDepartmentLabel(dept).toLowerCase().includes(searchLower)
                );
        ...
        // Apply type filter
        if (typeFilter !== 'all') {
            if (event.extendedProps.status !== typeFilter) return false;
        }
    ...
    const getDayCellClass = (date) => {
        ...
        if (dayEvents.length > 0) {
            ...
            return statusObj ? statusObj.dayCellColor : "bg-white border-gray-200 hover:bg-gray-50";
        }
    ...
    const handleDepartmentToggle = (departmentValue) => {
        setFormData(prev => {
            ...
            const isSelected = currentDepartments.includes(departmentValue)

            if (isSelected) {
                // Remove department
                return {
                    ...prev,
                    departments: currentDepartments.filter(dept => dept !== departmentValue)
                }
            } else {
                // Add department
                return {
                    ...prev,
                    departments: [...currentDepartments, departmentValue]
                }
            }
        })
    }
    ...
    const getDepartmentColors = () => {
        return formData.departments.map(deptValue => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-4">
            {/* Notification */}
            {notification.show && (
                <div className={`fixed top-4 right-4 z-50 p-4 rounded-lg shadow-lg border-l-4 ${notification.type === "error"
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                                            ...
                                            <div
                                                ...
                                                onClick={() => {
                                                    ...
                                                    setFormData({
                                                        ...
                                                        endTime: event.extendedProps.endTime || "17:00",
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <Badge variant="secondary" className={`${getEventColor(event.extendedProps.status)}-100 text-${getEventColor(event.extendedProps.status)}-800`}>
                    ...
                    <div className="lg:col-span-3">
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            ...
                            {calendarReady && plugins.dayGridPlugin && plugins.interactionPlugin ? (
                                <FullCalendar
                                    ref={calendarRef}
                                    plugins={[plugins.dayGridPlugin, plugins.interactionPlugin]}
                                ...
                                {formData.departments.length > 0 && (
                                    <div className="mt-3 p-3 bg-blue-50 rounded-lg">
                                        <p className="text-sm text-blue-700 font-medium">
                                            Selected: {formData.departments.map(getDepartmentLabel).join(", ")}
                                        </p>
                                    </div>
                                )}
                            </div>
                                ...
                                <div className="space-y-2">
                                    {statusOptions.map(status => (
                                        <button
                                            ...
                                            disabled={loading}
                                            className={`w-full p-3 rounded-lg border-2 transition-all flex items-center gap-3 ${formData.status === status.value
                                                ? "border-blue-500 bg-blue-50"
                                                : "border-gray-200 hover:border-gray-300"
                                                } ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
                                        >
