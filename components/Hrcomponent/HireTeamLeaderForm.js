...
export default function HireTeamLeaderForm({ onClose, onSuccess, currentHr }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/api/hr/team-leaders/hire`, {
                method: "POST",
                headers: {
            ...
            if (response.ok) {
                ...
                setModalMessage(`${formData.name} has been successfully added as a Team Leader!`)
                ...
                throw new Error(errorData.message || "Failed to hire Team Leader")
            }
        } catch (error) {
            setModalMessage(error.message || "Failed to hire Team Leader. Please try again.")
                            ...
                            <div className="flex items-center justify-between relative">
                                {steps.map((step, index) => (
                                    ...
                                    >
                                        ...
                                        <p
                                            className={`text-xs mt-2 text-center font-medium max-w-20 ${currentStep >= step.number ? "text-blue-600" : "text-gray-500"
                                                }`}
                                        >
                                            {step.title}
                                        </p>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <p className="text-xs text-gray-500 mt-2 bg-gray-50 p-2 rounded">
                                        Team Leader will be asked to change this on first login
                                    </p>
                                </div>
                            </div>
                        )}
