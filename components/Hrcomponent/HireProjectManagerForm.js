...
export default function HireProjectManagerForm({ onClose, onSuccess}) {
  ...
  const validateStep = (step) => {
    ...
    switch (step) {
      ...
      case 4:
        ...
        if (!formData.assignedDomain) {
          errors.assignedDomain = "Assigned domain is required"
          isValid = false
        }
        if (!formData.monthlyProjectTarget) {
          errors.monthlyProjectTarget = "Monthly project target is required"
          isValid = false
        }
        break
      default:
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/hr/project-manager/hire`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        setModalMessage(`${formData.name} has been successfully added as a Project Manager!`)
        ...
        throw new Error(errorData.message || "Failed to hire Project Manager")
      }
    } catch (error) {
      setModalMessage(error.message || "Failed to hire Project Manager. Please try again.")
            ...
            <div className="mt-8 mb-4">
              <div className="flex items-center justify-between relative">
                {steps.map((step, index) => (
                  <div
                    key={step.number}
                    className="flex flex-col items-center z-10 bg-gradient-to-r from-blue-50 to-indigo-50 px-2"
                  >
                    <div
          ...
          <CardContent className="p-8">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500 mt-2 bg-gray-50 p-2 rounded">
                    Project Manager will be asked to change this on first login
                  </p>
                </div>
              </div>
            )}
            ...
            {currentStep === 2 && (
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div
                    className={`border-2 border-dashed rounded-lg p-6 text-center transition-colors ${formErrors.photo ? "border-red-300 bg-red-50" : "border-gray-300 hover:border-blue-400 hover:bg-blue-50"}`}
                  >
              ...
              <div className="space-y-6">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="font-medium text-blue-900">Bank Account Verification</h4>
            ...
            {currentStep === 4 && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label htmlFor="assignedDomain" className="text-sm font-medium text-gray-700">
                      Assigned Domain *
                    </Label>
                    <Input
                      id="assignedDomain"
                      value={formData.assignedDomain}
                      onChange={(e) => handleInputChange("assignedDomain", e.target.value)}
                      placeholder="e.g., Web Development, Mobile Apps, AI/ML"
                      className="h-12 border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                      aria-invalid={!!formErrors.assignedDomain}
                    />
                    {formErrors.assignedDomain && (
                      <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                        <AlertCircle className="w-3 h-3" />
                        {formErrors.assignedDomain}
                      </p>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="monthlyProjectTarget" className="text-sm font-medium text-gray-700">
                      Monthly Project Target *
                    </Label>
                    <Input
                      id="monthlyProjectTarget"
                      type="number"
                      value={formData.monthlyProjectTarget}
                      onChange={(e) => handleInputChange("monthlyProjectTarget", e.target.value)}
                      placeholder="5"
                      className="h-12 border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                      aria-invalid={!!formErrors.monthlyProjectTarget}
                    />
                    {formErrors.monthlyProjectTarget && (
                      <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                        <AlertCircle className="w-3 h-3" />
                        {formErrors.monthlyProjectTarget}
                      </p>
            ...
            <div className="flex justify-between items-center pt-8 border-t mt-8">
              ...
              {currentStep < 4 ? (
                <Button onClick={nextStep} className="flex items-center gap-2 h-12 px-8 bg-blue-500 hover:bg-blue-600">
                  Next
