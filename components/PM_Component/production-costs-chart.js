...
export default function ProductionCostsChart() {
  ...
  return (
    <Card className="bg-white">
      <CardHeader>
        <CardTitle className="text-lg font-semibold text-gray-900">Production Costs</CardTitle>
        <div className="space-y-2 text-sm">
          <div>
            <span className="text-gray-600">Total Production</span>
            <div className="font-semibold">₹{(totalProduction / 100).toFixed(2)} PA</div>
          </div>
          <div>
            <span className="text-gray-600">Total Overhead</span>
            <div className="font-semibold">₹{(totalOverhead / 100).toFixed(2)} PA</div>
          </div>
          <div>
            <span className="text-gray-600">Maintenance costs</span>
            <div className="font-semibold">₹{maintenanceCosts.toLocaleString()}</div>
          </div>
        </div>
      </CardHeader>
      <CardContent>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <Area
                ...
                fillOpacity={0.6}
              />
              <Area type="monotone" dataKey="overhead" stackId="1" stroke="#8b5cf6" fill="#8b5cf6" fillOpacity={0.6} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
