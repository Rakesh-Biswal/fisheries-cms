    ...
    try {
      const payload = {
        ...
        slots: slots.map(slot => ({
          ...
          endTime: slot.endTime || `${parseInt(slot.startTime.split(':')[0]) + 1}:00`
        })),
      ...
      // Add to local meeting history for immediate UI update
      const newMeetings = payload.slots.map((slot, index) => ({
        id: `${created.id}_${index}`,
        title: created.title,
        date: slot.date,
        startTime: slot.startTime,
        endTime: slot.endTime,
        room: meetingRooms[0], // Default room, can be selected in future enhancement
  ...
  return (
    <DashboardLayout title="Meeting Management">
      ...
      <Dialog open={showCalendar} onOpenChange={setShowCalendar}>
        <DialogContent className="max-w-70xl max-h-[900vh] overflow-y-auto">
          <DialogHeader>
