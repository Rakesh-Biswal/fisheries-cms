...
export default function TeamLeadersPage() {
  ...
  // ✅ New states for team details modal
  ...
  const handleViewDetails = (team) => {
    setSelectedTeam(team) // ✅ open modal with that team
  }
  ...
  const handleAssignEmployee = () => {
    ...
    setTeams((prevTeams) =>
      prevTeams.map((team) =>
        team.id === selectedTeam.id
          ? {
              ...team,
              workers: [...(team.workers || []), newEmployee],
              size: (team.size || 0) + 1,
            }
          : team
      )
    ...
    toast.success(`${newEmployee} assigned to ${selectedTeam.name} team!`)
    setNewEmployee("")
  }
