...
// Fetch meeting history from backend - UPDATED
useEffect(() => {
...
// Get meetings for specific date and room - UPDATED
...
// Get all meetings for selected date - UPDATED
...
// Get employee name by ID - UPDATED
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const payload = {
      ...
      googleMeetLink: googleMeetLink, // ✅ Match backend field name
      description: notes, // ✅ Match backend field name
    ...
    // ✅ Use absolute URL for API calls to avoid path issues
    const API_BASE_URL = process.env.NODE_ENV === 'production' 
      ? 'https://your-production-url.com' 
      : 'http://localhost:5000';
...
const calculateEndTime = (startTime, duration = 60) => {
  ...
  return (
    <DashboardLayout title="Meeting Management">
      ...
      {/* Calendar Modal */}
     // Calendar Modal - Updated to show real data
            ...
            <div className="p-4 border-r bg-gray-50 font-medium flex items-center justify-between">
              ...
              <Badge variant="outline" className="text-xs">
                {getMeetingsForSelectedDate().filter(m => {
                  // Match meetings that occur at this time slot
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {selectedMeeting.participants?.map((participant, index) => (
                  <Badge key={participant.employeeId || index} variant="outline" className="flex items-center gap-1">
