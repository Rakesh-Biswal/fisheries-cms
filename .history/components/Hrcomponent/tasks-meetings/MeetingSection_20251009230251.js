...
export default function MeetingSection() {
  ...
  const fetchMeetings = async () => {
    try {
      setLoading(true)
      const response = await fetch(`http://localhost:5000/api/hr/meetings`, {
        credentials: 'include'
      })
...
const testAuth = async () => {
  try {
    ...
    const response = await fetch(`${API_URL}/api/hr/meetings/debug-auth`, {
      credentials: 'include',
      headers: {
...
// Call this in useEffect to test authentication
useEffect(() => {
  testAuth(); // Add this line
  fetchMeetings();
  fetchMeetingStats();
  fetchDepartments();
}, []);
...
const fetchDepartments = async () => {
  try {
    ...
    if (response.ok) {
      const data = await response.json()
      setDepartments(data.data || HARDCODED_DEPARTMENTS) // Fallback to hardcoded
    } else {
...
// In MeetingSection.js, update the handleCreateMeeting function:
