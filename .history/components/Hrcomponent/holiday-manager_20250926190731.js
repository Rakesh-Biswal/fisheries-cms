...
const ProfessionalAttendanceCalendar = () => {
  ...
  // Fetch events from backend or use mock data
  const fetchEvents = async () => {
    try {
      ...
      // Try real API call
      const result = await apiFetch(`${API_BASE_URL}/hr/attendance-calendar/fetch`);
      ...
      setUseMockData(true);
      setEvents(MOCK_EVENTS);
      showNotification("Using demo data - Could not connect to server", "warning");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getDisplayTime = () => {
    ...
    if (!formData.startTime || !formData.endTime) return "Time not set"
  ...
  // Create event (with mock fallback)
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (useMockData) {
        ...
        // Real API call
        result = await apiFetch(`${API_BASE_URL}/hr/attendance-calendar/create`, {
          method: "POST",
  ...
  // Update event (with mock fallback)
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (useMockData) {
        ...
        // Real API call
        result = await apiFetch(`${API_BASE_URL}/hr/attendance-calendar/${existingEvent.id}`, {
          method: "PUT",
          body: JSON.stringify(eventData),
        });
      }
  ...
  // Delete event (with mock fallback)
            ...
            <div className="flex items-center gap-4 mb-4 lg:mb-0">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  {useMockData ? "Demo Mode - Using sample data" : "Manage holidays, leaves, and working days efficiently"}
                </p>
                {useMockData && (
                  <p className="text-sm text-yellow-600 mt-1">
                    Backend server not available. Events are stored locally.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              {useMockData && (
                <div className="bg-yellow-50 px-3 py-2 rounded-lg border border-yellow-200">
                  <span className="text-yellow-700 font-medium text-sm">Demo Mode</span>
                </div>
              )}
              <div className="bg-green-50 px-3 py-2 rounded-lg">
                <span className="text-green-700 font-medium">{events.length} Events</span>
              </div>
              <button 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="flex items-center gap-2">
                <div className={`w-3 h-3 rounded-full ${useMockData ? 'bg-yellow-500' : 'bg-green-500'}`}></div>
                <span className="text-sm text-gray-700">
                  {useMockData ? 'Using Demo Data' : 'Connected to Server'}
                </span>
              </div>
              <p className="text-xs text-gray-500 mt-2">
                {useMockData 
                  ? 'Backend server not available. Data is stored locally.'
                  ...
                  eventContent={(eventInfo) => (
                    <div className="p-1">
                      ...
                      <div className="text-xs opacity-75 truncate flex items-center gap-1">
                        <Clock size={10} />
                        {eventInfo.event.extendedProps?.displayTime || 'All Day'}
                      </div>
            ...
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-6 text-white">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  {useMockData && (
                    <p className="text-blue-200 text-xs mt-1">Demo Mode - Changes are local</p>
                  )}
                </div>
                <button
                  onClick={() => {
