...
const ProfessionalAttendanceCalendar = () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const transformedEvents = result.data.map(event => ({
          ...
          extendedProps: {
            departments: event.departments, // Now handling array of departments
            status: event.status,
  ...
  // Updated departments list with all required departments
   const departments = [
  ...
  const handleDateClick = (info) => {
    ...
    if (existing) {
      ...
      setIsDateUsed(true)
      setFormData({
        departments: existing.extendedProps.departments || [], // Now handling array
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const eventData = {
        ...
        date: selectedDate,
        departments: formData.departments, // Now sending array
        status: formData.status,
        startTime: formData.startTime,
        endTime: formData.endTime,
        displayTime: getDisplayTime(),
        departmentColors: getDepartmentColors(), // Now array of colors
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {eventInfo.event.extendedProps?.departments?.map((dept, index) => {
                        ...
                        return (
                          <div 
                            key={index} 
                            className={`text-xs px-1 py-0.5 rounded ${colorClass}`}
                          >
