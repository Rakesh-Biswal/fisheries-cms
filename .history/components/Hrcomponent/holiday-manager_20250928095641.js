...
// Handle department selection - UPDATED with better logging
const handleDepartmentToggle = (departmentValue) => {
  console.log("Before toggle - current departments:", formData.departments);
  ...
  setFormData(prev => {
    ...
    console.log("After toggle - new departments:", newDepartments);
    return {
      ...prev,
...
const handleSubmit = async () => {
  ...
  try {
    ...
    console.log("Departments array:", formData.departments);
    console.log("Departments array length:", formData.departments.length);
    console.log("Departments array content:", JSON.stringify(formData.departments));

// In your handleSubmit function, replace the fetch call with:
const response = await fetch(`${API_BASE_URL}/api/hr/attendance-calendar/create`, {
  ...
  body: JSON.stringify({
    ...eventData,
    departments: Array.isArray(formData.departments) ? formData.departments : [formData.departments]
  }),
});
    const result = await response.json();
