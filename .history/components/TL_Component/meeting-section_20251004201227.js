...
export default function MeetingSection() {
  ...
  // Get first featured meeting (upcoming)
  ...
  const pastMeeting = pastMeetings[0];
        ...
        <CardContent className="space-y-6">
          ...
          {featuredMeeting && (
            <div className="border rounded-lg p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  ...
                  {featuredMeeting.timeSlots?.[0] ? 
                    `${formatDate(featuredMeeting.timeSlots[0].date)} • ${formatTimeTo12Hour(featuredMeeting.timeSlots[0].startTime)} - ${formatTimeTo12Hour(featuredMeeting.timeSlots[0].endTime)}` : 
                    'Time not set'
                  }
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-600">
              ...
              {/* Show more featured meetings if available */}
              {upcomingMeetings.length > 1 && !showAllFeatured && (
                <Button
                  variant="ghost"
                  size="sm"
              ...
              {/* Show all featured meetings when expanded */}
              {showAllFeatured && upcomingMeetings.slice(1).map(meeting => (
              ...
              {showAllFeatured && upcomingMeetings.length > 1 && (
                <Button
                  variant="ghost"
                  size="sm"
                  className="w-full mt-2 text-xs text-blue-600 hover:text-blue-700"
          ...
          {ongoingMeeting && (
            <div className="border rounded-lg p-4 bg-gradient-to-r from-green-50 to-emerald-50 border-green-200">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  <Clock className="w-3 h-3" />
                  {ongoingMeeting.timeSlots?.[0] ? 
                    `${formatDate(ongoingMeeting.timeSlots[0].date)} • ${formatTimeTo12Hour(ongoingMeeting.timeSlots[0].startTime)} - ${formatTimeTo12Hour(ongoingMeeting.timeSlots[0].endTime)}` : 
                    'Time not set'
                  }
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  <Users className="w-3 h-3" />
                  {ongoingMeeting.participants?.length || 0} participants
                </div>
              </div>
              ...
              >
                <a 
                  href={ongoingMeeting.googleMeetLink} 
                  target="_blank" 
                  rel="noreferrer"
                  className="flex items-center justify-center gap-2"
                >
              ...
              {ongoingMeetings.length > 1 && !showAllOngoing && (
                <Button
                  variant="ghost"
                  size="sm"
              ...
              {showAllOngoing && ongoingMeetings.slice(1).map(meeting => (
              ...
              {showAllOngoing && ongoingMeetings.length > 1 && (
                <Button
                  variant="ghost"
                  size="sm"
                  className="w-full mt-2 text-xs text-green-600 hover:text-green-700"
          ...
          {pastMeeting && (
            <div className="border rounded-lg p-4 bg-gray-50 border-gray-200">
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-semibold text-gray-700">Meeting History</h4>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  <Clock className="w-3 h-3" />
                  {pastMeeting.timeSlots?.[0] ? 
                    `${formatDate(pastMeeting.timeSlots[0].date)} • ${formatTimeTo12Hour(pastMeeting.timeSlots[0].startTime)} - ${formatTimeTo12Hour(pastMeeting.timeSlots[0].endTime)}` : 
                    'Time not set'
                  }
                </div>
                <div className="flex items-center gap-2 text-xs text-gray-600">
                  ...
                  {pastMeeting.participants?.length || 0} attended
                </div>
              </div>
          ...
          {!featuredMeeting && !ongoingMeeting && !pastMeeting && !loading && (
