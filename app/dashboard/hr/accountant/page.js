      ...
      {showHireForm ? (
        <div className="bg-white p-2 w-full">
          <div className="rounded-lg">
            <HireAccountantForm onClose={() => setShowHireForm(false)} onSuccess={handleHireSuccess} />
          </div>
        </div>
      ) : (
        ...
        <div className="space-y-6 p-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground">Manage accounting staff and their responsibilities</p>
            </div>
          ...
          {error ? (
            ...
            <Card className="text-center p-8">
              ...
              <p className="text-muted-foreground mb-4">
                Start by hiring your first accountant to manage financial operations.
              </p>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {accountants.map((accountant) => (
                  <Card key={accountant._id} className="hover:shadow-lg transition-shadow">
                    <CardHeader className="pb-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <CardDescription>{accountant.businessData?.designation}</CardDescription>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent className="space-y-3">
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-medium">{accountant.experience || 0} years</span>
                      </div>
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-muted-foreground">Qualification:</span>
                        <span className="font-medium">{accountant.qualification || "N/A"}</span>
                      </div>
                      <div className="flex items-center justify-between text-sm">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Summary of all accountants and their information</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="text-center p-4 bg-green-50 rounded-lg">
                      <div className="text-2xl font-bold text-green-600">{accountants.length}</div>
                      <div className="text-sm text-muted-foreground">Total Accountants</div>
                    </div>
                    <div className="text-center p-4 bg-blue-50 rounded-lg">
                      <div className="text-2xl font-bold text-blue-600">
                        {accountants.filter((a) => a.status === "active").length}
                      </div>
                      <div className="text-sm text-muted-foreground">Active Accountants</div>
                    </div>
                    <div className="text-center p-4 bg-orange-50 rounded-lg">
                      <div className="text-2xl font-bold text-orange-600">
                        {accountants.reduce((sum, accountant) => sum + (accountant.experience || 0), 0)}
                      </div>
                    ...
                    <div className="text-center p-4 bg-purple-50 rounded-lg">
                      <div className="text-2xl font-bold text-purple-600">
                        ₹
                        {accountants
                          .reduce((sum, accountant) => sum + (accountant.businessData?.salary || 0), 0)
                          .toLocaleString()}
                      </div>
